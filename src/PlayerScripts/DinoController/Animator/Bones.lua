...
function Bone.new(Joint0:classes.JointClass, Joint1:classes.JointClass)
    ...
    local DefaultRotation = p0:ToObjectSpace(CFrame.lookAt(p0.Position,p1)).Rotation:Inverse()
...
function Bone:UpdateP0(weight)
    ...
    self.Connection0:SetCFrame(Point0, weight)
end


function Bone:UpdateP1(weight)
    local p0 = self.Connection0:GetCFrame()
    local p1 = self.Connection1:GetCFrame()
    ...
    self.Connection1:SetCFrame(Point1, weight)
end
